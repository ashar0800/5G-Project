...
# Allocation thresholds for predicted utilization
HIGH_UTILIZATION_THRESHOLD = 85.0  # Percentage: if predicted utilization is above this, consider increasing resources
LOW_UTILIZATION_THRESHOLD = 30.0   # Percentage: if predicted utilization is below this, consider decreasing resources
...
def decide_allocation_action(cell_id, predicted_utilization, prediction_timestamp_str):
    ...
    reason = f"Predicted utilization {predicted_utilization:.2f}% is within normal operating thresholds ({LOW_UTILIZATION_THRESHOLD:.2f}% - {HIGH_UTILIZATION_THRESHOLD:.2f}%)."

    if predicted_utilization > HIGH_UTILIZATION_THRESHOLD:
        action = "SUGGEST_INCREASE_RESOURCES"
        reason = f"Predicted utilization {predicted_utilization:.2f}% exceeded high threshold ({HIGH_UTILIZATION_THRESHOLD:.2f}%)."
    elif predicted_utilization < LOW_UTILIZATION_THRESHOLD:
        action = "SUGGEST_DECREASE_RESOURCES"
        reason = f"Predicted utilization {predicted_utilization:.2f}% is below low threshold ({LOW_UTILIZATION_THRESHOLD:.2f}%)."

    # Timestamp for when this allocation decision is being made
    current_decision_timestamp = datetime.utcnow().isoformat() + "Z"
